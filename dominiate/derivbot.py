...
def buy_value(coins, buys, order):
    value = 0
    for j in range(buys):
...
class DerivBot(HillClimbBot):
        ...
        for deriv in (1, 2):
            ...
            for idx in range(self.k):
                ...
                for coins, buys in state.simulate_hands(1, hand):
                    actual_value = buy_value(coins, buys, prev_order)
                if coins >= 8:
                ...
                for card in game.card_choices():
                    for i in range(len(hand)):
                        ...
                        for coins, buys in state.simulate_hands(1, newhand):
                            value = buy_value(coins, buys, prev_order) - actual_value
    ...
    def buy_priority(self, decision, card):
        if card is None:
            ...
            return self.current_values[card]
